...
pub fn has_top_right(/*const AV1_COMMON *cm,*/
  ...
  if !top_available || !right_available { return false };
  ...
  if row_off > 0 {  // Just need to check if enough pixels on the right.
    ...
    if col_off + top_right_count_unit < plane_bw_unit { return true };
    ...
    if blk_row_in_sb == 0 { return true };

    // Rightmost column of superblock (and not the top row): so top-right pixels
    // fall in the right superblock, which is not available yet.
    if ((blk_col_in_sb + 1) << bw_in_mi_log2) >= sb_mi_size { return false };
...
static has_bl_vert_tables: &'static [&'static [u8]] = &[
  ...
  has_null,
  // 4X8,     8X4,         8X8
  has_bl_4x8, has_null, has_bl_vert_8x8,
  // 8X16,    16X8,        16X16
  has_bl_8x16, has_null, has_bl_vert_16x16,
  // 16X32,   32X16,       32X32
  has_bl_16x32, has_null, has_bl_vert_32x32,
  // 32X64,   64X32,       64X64
  has_bl_32x64, has_null, has_bl_vert_64x64,
  // 64x128,  128x64,      128x128
  has_bl_64x128, has_null, has_bl_128x128
];
...
/*static int has_bottom_left(const AV1_COMMON *cm, BLOCK_SIZE bsize, int mi_row,
  ...
  if !bottom_available || !left_available { return false };
  ...
  if bsize.width() > BLOCK_64X64.width() && col_off > 0 {
    ...
    if col_off_64 == 0 {
      ...
      let plane_bh_unit =
          (bsize.height_mi() >> ss_y).min(plane_bh_unit_64);
  ...
  if col_off > 0 {
    ...
    if ((blk_row_in_sb + 1) << bh_in_mi_log2) >= sb_mi_size { return false };
