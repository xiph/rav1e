...
pub fn cdef_analyze_superblock<T: Pixel>(
  ...
  for by in 0..8 {
    for bx in 0..8 {
      ...
      if global_block_offset.x < blocks.cols()
        && global_block_offset.y < blocks.rows()
      {
        ...
        if !skip {
          ...
          dir.dir[bx][by] = cdef_find_dir(
            &in_slice.reslice(8 * bx as isize + 2, 8 * by as isize + 2),
            &mut var,
            coeff_shift,
          ) as u8;
          dir.var[bx][by] = var;
        ...
        if !skip {
          ...
          for p in 0..3 {
            ...
            unsafe {
              ...
              assert!(
                in_slice.rows_iter().len() >= ((8 * by) >> ydec) + ysize + 4
              );
              let dst =
