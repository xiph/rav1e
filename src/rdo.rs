  ...
  for &luma_mode in mode_set {
    ...
    if is_chroma_block && fi.config.speed <= 3 {
      ...
      for &chroma_mode in RAV1E_INTRA_MODES {
        encode_block(fi, fs, cw, luma_mode, chroma_mode, bsize, bo, skip, cdef_index);
      ...
      encode_block(fi, fs, cw, luma_mode, luma_mode, bsize, bo, skip, cdef_index);
    ...
    match partition {
      PartitionType::PARTITION_NONE => {
        ...
        let mode_decision = cached_block
          .part_modes
          .get(0)
          .unwrap_or(&rdo_mode_decision(fi, fs, cw, bsize, bo, cdef_index).part_modes[0])
          .clone();
      ...
      PartitionType::PARTITION_SPLIT => {
        ...
        let mode_decision = rdo_mode_decision(fi, fs, cw, subsize, &offset, cdef_index)
          .part_modes[0]
          .clone();
        child_modes.push(mode_decision);
