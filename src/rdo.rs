...
fn compute_distortion<T: Pixel>(
  ...
  let mut distortion = match fi.config.tune {
    Tune::Psychovisual if w_y >= 8 && h_y >= 8 => cdef_dist_wxh(
      &input_region,
      &rec_region,
      w_y,
      h_y,
    ...
    Tune::Psnr | Tune::Psychovisual => {
      sse_wxh(&input_region, &rec_region, w_y, h_y)
    }
  };
      ...
      for p in 1..3 {
        distortion += sse_wxh(
          &ts.input_tile.planes[p]
            .subregion(Area::BlockStartingAt { bo: tile_bo }),
      ...
      let mut chroma_rdo = |skip: bool| {
        mode_set_chroma.iter().for_each(|&chroma_mode| {
          ...
          let distortion = if fi.use_tx_domain_distortion && !need_recon_pixel
          {
            ...
            compute_distortion(fi, ts, w, h, is_chroma_block, tile_bo, false)
          };
    ...
    let mut sads = {
      ...
      intra_mode_set
        ...
        .map(|&luma_mode| {
          ...
          let mut rec_region =
            rec.subregion_mut(Area::BlockStartingAt { bo: tile_bo });
...
pub fn rdo_tx_type_decision<T: Pixel>(
  ...
  for &tx_type in tx_types {
    ...
    let distortion = if fi.use_tx_domain_distortion {
      ...
      compute_distortion(fi, ts, w, h, is_chroma_block, tile_bo, true)
    };
    let rd = compute_rd_cost(fi, rate, distortion);
    if rd < best_rd {
      best_rd = rd;
    ...
    match partition {
      ...
      PARTITION_SPLIT | PARTITION_HORZ | PARTITION_VERT => {
        ...
        let four_partitions = [
          ...
          BlockOffset {
            x: tile_bo.x + hbsw as usize,
            y: tile_bo.y + hbsh as usize,
          },
        ];
    ...
    for bx in 0..sb_blocks {
      ...
      if bo.x < blocks.cols() && bo.y < blocks.rows() {
        ...
        if !skip {
          ...
          let in_region =
            in_plane.subregion(Area::BlockStartingAt { bo: in_bo });

          let test_bo = sbo_0.block_offset(bx << 1, by << 1);
          let test_region =
            test_plane.region(Area::BlockStartingAt { bo: test_bo });
          ...
          for pli in 0..3 {
            match best_lrf[pli] {
              RestorationFilter::None {} => {
                ...
                cost[pli] = err as f64
                  + fi.lambda * rate as f64 / ((1 << OD_BITRES) as f64);
                cost_acc += cost[pli];
              }
      ...
      && ts.restoration.has_restoration_unit(tile_sbo)
    {
      ...
      for pli in 0..3 {
        ...
        for set in 0..16 {
          ...
          let cost =
            err as f64 + fi.lambda * rate as f64 / ((1 << OD_BITRES) as f64);
