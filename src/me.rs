...
fn get_mv_range(
  ...
  let mvx_max = (w_in_b - bo.x - blk_w / MI_SIZE) as isize
    * (8 * MI_SIZE) as isize
    + border_w;
  let mvy_min = -(bo.y as isize) * (8 * MI_SIZE) as isize - border_h;
  let mvy_max = (h_in_b - bo.y - blk_h / MI_SIZE) as isize
    * (8 * MI_SIZE) as isize
    + border_h;
...
pub fn get_subset_predictors<T: Pixel>(
  ...
  if let Some(ref frame_ref) = frame_ref_opt {
    ...
    let frame_bo =
      BlockOffset { x: tile_mvs.x() + tile_bo.x, y: tile_mvs.y() + tile_bo.y };
    if frame_bo.x > 0 {
...
impl MotionEstimation for FullSearch {
  ...
  fn me_ss2<T: Pixel>(
    ...
    pmvs: &[Option<MotionVector>; 3],
    tile_bo_adj: BlockOffset,
    _tile_mvs: &TileMotionVectors<'_>,
    _frame_ref_opt: Option<&ReferenceFrame<T>>,
    rec: &ReferenceFrame<T>,
    _global_mv: [MotionVector; 2],
    lambda: u32,
    mvx_min: isize,
    mvx_max: isize,
    mvy_min: isize,
...
fn get_best_predictor<T: Pixel>(
  ...
  tmp_plane_opt: &mut Option<Plane<T>>,
  ref_frame: RefType,
) {
...
fn adjust_bo(
  ...
  BlockOffset {
    x: (bo.x as isize).min(mi_width as isize - blk_w as isize / 4).max(0)
      as usize,
    y: (bo.y as isize).min(mi_height as isize - blk_h as isize / 4).max(0)
      as usize,
  }
}
