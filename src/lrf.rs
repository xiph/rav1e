...
fn sgrproj_sum_finish(
  ...
  let p =
    cmp::max(0, scaled_ssq * (n as i32) - scaled_sum * scaled_sum) as u32;
  let z =
    (p * s as u32 + (1 << SGRPROJ_MTABLE_BITS >> 1)) >> SGRPROJ_MTABLE_BITS;
  let a: i32 = if z >= 255 {
    256
  } else if z == 0 {
    1
...
fn sgrproj_box_sum_slow<T: Pixel>(
  a: &mut i32,
  b: &mut i32,
  stripe_y: isize,
  stripe_h: usize,
...
fn sgrproj_box_sum_fastxy_r1<T: Pixel>(
  a: &mut i32,
  b: &mut i32,
  x: isize,
  y: isize,
...
fn sgrproj_box_sum_fastxy_r2<T: Pixel>(
  a: &mut i32,
  b: &mut i32,
  x: isize,
  y: isize,
...
fn sgrproj_box_sum_fastx_r1<T: Pixel>(
  a: &mut i32,
  b: &mut i32,
  stripe_y: isize,
  stripe_h: usize,
  ...
  for yi in y - 1..=y + 1 {
    ...
    let (src_plane, src_h) =
      if yi >= stripe_y && yi < stripe_y + stripe_h as isize {
        (cdeffed, cdeffed_h as isize)
      } else {
        (backing, backing_h as isize)
      };
...
fn sgrproj_box_sum_fastx_r2<T: Pixel>(
  a: &mut i32,
  b: &mut i32,
  stripe_y: isize,
  stripe_h: usize,
...
fn sgrproj_box_ab_r1<T: Pixel>(
  ...
  if backing.x + stripe_x > 0
    && stripe_x < backing_w as isize - 1
    && cdeffed.x + stripe_x > 0
    && stripe_x < cdeffed_w as isize - 1
  {
    ...
    let boundary2 =
      cmp::min(cdeffed_h as isize - stripe_y - 1, stripe_h as isize - 1)
        as usize;

    // top rows (if any), away from left and right columns
    for i in boundary0..boundary1 {
      sgrproj_box_sum_fastx_r1(
        &mut af[i],
        &mut bf[i],
        stripe_y,
        stripe_h,
        stripe_x,
        stripe_y + i as isize - 1,
        s,
        bdm8,
        backing,
        backing_h,
...
fn sgrproj_box_ab_r2<T: Pixel>(
  ...
  if backing.x + stripe_x > 1
    && stripe_x < backing_w as isize - 2
    && cdeffed.x + stripe_x > 1
    && stripe_x < cdeffed_w as isize - 2
  {
    // Addressing is away from left and right edges of cdeffed storage;
    // no X clipping to worry about, but the top/bottom few rows still
    // need to worry about storage and stripe limits
...
fn sgrproj_box_f_r0<T: Pixel>(
  ...
  for i in cmp::max(0, -y) as usize..h {
    f[i as usize] = (i32::cast_from(cdeffed.p(x, (y + i as isize) as usize)))
      << SGRPROJ_RST_BITS;
  }
}
...
fn sgrproj_box_f_r1<T: Pixel>(
  ...
  for i in cmp::max(0, -y) as usize..h {
    let a = 3 * (af[0][i] + af[2][i] + af[0][i + 2] + af[2][i + 2])
      + 4
        * (af[1][i]
          + af[0][i + 1]
          + af[1][i + 1]
          + af[2][i + 1]
          + af[1][i + 2]);
    let b = 3 * (bf[0][i] + bf[2][i] + bf[0][i + 2] + bf[2][i + 2])
      + 4
        * (bf[1][i]
          + bf[0][i + 1]
          + bf[1][i + 1]
...
fn sgrproj_box_f_r2<T: Pixel>(
  ...
  for i in (cmp::max(0, -y) as usize..h).step_by(2) {
    let a = 5 * (af[0][i] + af[2][i]) + 6 * (af[1][i]);
    let b = 5 * (bf[0][i] + bf[2][i]) + 6 * (bf[1][i]);
    let ao = 5 * (af[0][i + 2] + af[2][i + 2]) + 6 * (af[1][i + 2]);
    let bo = 5 * (bf[0][i + 2] + bf[2][i + 2]) + 6 * (bf[1][i + 2]);
    let v = (a + ao) * i32::cast_from(cdeffed.p(x, (y + i as isize) as usize))
      + b
      + bo;
    f[i as usize] = (v + (1 << shift >> 1)) >> shift;
    let vo =
      ao * i32::cast_from(cdeffed.p(x, (y + i as isize) as usize + 1)) + bo;
...
pub fn sgrproj_stripe_filter<T: Pixel>(
  ...
  for xi in 0..stripe_w {
    ...
    if s_r2 > 0 {
      ...
      let ap0: [&[i32; 64 + 2]; 3] =
        [&a_r2[xi % 3], &a_r2[(xi + 1) % 3], &a_r2[(xi + 2) % 3]];
      let bp0: [&[i32; 64 + 2]; 3] =
        [&b_r2[xi % 3], &b_r2[(xi + 1) % 3], &b_r2[(xi + 2) % 3]];
      sgrproj_box_f_r2(
        &ap0,
        &bp0,
        &mut f_r2,
    ...
    if s_r1 > 0 {
      ...
      let ap1: [&[i32; 64 + 2]; 3] =
        [&a_r1[xi % 3], &a_r1[(xi + 1) % 3], &a_r1[(xi + 2) % 3]];
      let bp1: [&[i32; 64 + 2]; 3] =
        [&b_r1[xi % 3], &b_r1[(xi + 1) % 3], &b_r1[(xi + 2) % 3]];
