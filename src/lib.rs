...
impl EncoderFiles {
    pub fn from_cli() -> EncoderFiles {
        ...
        let rec_file = if matches.is_present("RECONSTRUCTION") {
            Some(File::create(matches.value_of("RECONSTRUCTION").unwrap()).unwrap())
        } else {
            None
        };
...
pub fn process_frame(frame_number: u64, sequence: &Sequence, fi: &FrameInvariants,
                     ...
                     y4m_enc_option: Option<&mut y4m::Encoder<File>>) -> bool {
    let width = fi.width;
