...
fn deblock_size<T: Pixel>(
    ...
    || prev_block.ref_frames[0] == INTRA_FRAME)
  {
    ...
    let (txsize, prev_txsize) = if pli == 0 {
      ...
      (
        block.bsize.largest_uv_tx_size(xdec, ydec),
        prev_block.bsize.largest_uv_tx_size(xdec, ydec),
      )
    };
...
fn sse_plane<T: Pixel>(
  ...
  h_sse: &mut [i64; MAX_LOOP_FILTER + 2],
  pli: usize,
  blocks: &FrameBlocks,
  bd: usize,
) {
