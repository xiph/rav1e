...
pub fn encode_tx_block<T: Pixel>(
  ...
  if mode.is_intra() {
    ...
    let edge_buf =
      get_intra_edges(&rec.as_const(), po, tx_size, bit_depth, Some(mode));
    mode.predict_intra(
      tile_rect,
...
pub fn motion_compensate<T: Pixel>(
  ...
  for p in 0..num_planes {
    let plane_bsize =
      if p == 0 { bsize } else { get_plane_block_size(bsize, u_xdec, u_ydec) };
...
pub fn encode_block_with_modes<T: Pixel>(
  ...
  ts: &mut TileStateMut<'_, T>,
  cw: &mut ContextWriter,
  w_pre_cdef: &mut dyn Writer,
  w_post_cdef: &mut dyn Writer,
  bsize: BlockSize,
  tile_bo: BlockOffset,
  mode_decision: &RDOPartitionOutput,
  rdo_type: RDOType,
) {
