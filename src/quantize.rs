...
static OD_DIVU_SMALL_CONSTS: &[[u32; 2]] = &[
...
#[inline]
fn divu_small(x: i32, d: u32) -> i32 {
    let index = (d - 1) as usize;
    let y = if x < 0 {-x} else {x} as u64;
    let a = OD_DIVU_SMALL_CONSTS[index][0] as u64;
    let b = OD_DIVU_SMALL_CONSTS[index][1] as u64;

    let y = (((a * y + b) >> 32) >> (32 - d.leading_zeros() - 1)) as i32;
    if x < 0 { -y } else { y }
}
...
mod test {
    ...
    fn test_divu_small() {
        for d in 1..1024 {
            for x in -1000_000..1000_000 {
                assert_eq!(x/d, divu_small(x, d as u32));
            }
        }
    }
}
...
impl QuantizationContext {
    ...
    pub fn quantize(&self, coeffs: &mut [i32]) {
      ...
      let dc_offset = self.dc_quant * 21 / 64 as i32;
      let ac_offset = self.ac_quant * 21 / 64 as i32;

      coeffs[0] *= self.tx_scale;
      coeffs[0] += coeffs[0].signum() * dc_offset;
      coeffs[0] = divu_small(coeffs[0], self.dc_quant as u32);

      for c in coeffs[1..].iter_mut() {
        *c *= self.tx_scale;
        *c += c.signum() * ac_offset;
        *c = divu_small(*c, self.ac_quant as u32);
      }
    }
}
